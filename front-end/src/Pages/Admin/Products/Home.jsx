...
const fetchData = async (page) => {
  try {
    const result = await axios.get(`http://localhost:9002/api/products`);
...
const Home = () => {
  ...
  const deleteCategory = async (categoryID) => {
    ...
    await axios
      .delete(`http://localhost:9002/api/products/${categoryID}`)
      .then((response) => {
                  ...
                  {state.data.map((category) => (
                    <tr key={category._id}>
                      ...
                      <td className=" border px-4 py-2 flex items-center justify-start space-x-5">
                        <Link to={`/admin/editproducts?id=${category._id}`}>
